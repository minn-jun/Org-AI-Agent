...
기본적으로 최근 8턴만 유지한다. 그래서 대화가 길어져도 컨텍스트가 무한히 커지지 않는다.
...
- 제목에 있으면 추가 가산
- 프로젝트명에 있으면 추가 가산
- `오늘`, `아까`, `최근` 계열 질문이면 최신 문서에 가산
- `공식`, `최종`, `기준` 계열 질문이면 LTM 문서에 가산

prefetch 단계에서는 여기에 query analyzer의 tier 비율을 더해 rerank 점수를 만든다.

```text
rerank_score = retrieval_score + tier_weight * 5
```

즉, 문서 자체의 키워드 관련도와 “이번 질문에서 이 tier가 얼마나 중요하다고 판단됐는지”를 함께 본다.

단, STM 5개, MTM 2개, LTM 1개처럼 총 8개를 배분하더라도 항상 8개가 채워지는 것은 아니다. 각 tier 검색에서 점수가 0보다 큰 문서만 결과로 남고, 이후 같은 evidence는 중복 제거된다. 그래서 실제 컨텍스트 후보는 6개처럼 더 적을 수 있다.
