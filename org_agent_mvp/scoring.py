...
#: 2026-09-16 기본값을 1.2로 바꿨다. 위 18건 비교는 표본이 작았고,
#: Allganize dev 150문항에서는 1.2와 2.0이 동률(페이지 MRR 0.795 vs 0.788)이라
#: 선택 규칙(동률이면 관례값)에 따라 관례값 1.2를 쓴다. 05·07 문서.
K1 = 1.2
...
#: 2026-09-16 기본값을 BM25로 바꿨다. LTM은 Allganize 299문항에서 확실히 낫고(0.718 -> 0.814),
#: STM/MTM도 같이 BM25여야 계층 병합이 버틴다(07 문서 2절). 계층별로 다르게 주면 눈금이 벌어진다.
DEFAULT_SCORER = "bm25"
...
def scorer_name(scope: str = "ltm") -> str:
    ...
    코퍼스 크기가 100배 차이나면 같은 점수 함수가 양쪽에 다 맞지 않는다.
        ...
        STM/MTM    문서 85건. IDF가 거의 평평하고 길이도 고르다.

    예전에는 STM/MTM에 BM25가 나쁘다고 봤지만(합성 시드 MRR 0.864 -> 0.680), 2026-09-16
    20200504 60건 전체로 다시 재니 **양쪽을 같은 점수 함수로 두는 것이 더 중요했다** —
    LTM만 BM25면 계층 최고점 비가 1.86으로 벌어져 계층 혼합 MRR이 0.626 -> 0.350으로 무너진다(07 문서 2-3절).
    그래서 기본값은 둘 다 BM25다. 계층별로 다르게 주려면 아래 변수를 쓴다.
